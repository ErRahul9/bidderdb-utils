...
def date_factory(days: int = 0, timezone: pytz.UTC = pytz.utc):
...
def datetime_factory(days: int = 0, minutes: int = 0, timezone: pytz.UTC = pytz.utc):
    ...
    """
    new_datetime = datetime.now(timezone) + timedelta(days=days, minutes=minutes)
    return datetime(
...
def epoch_in_microseconds(days=0, minutes=0):
    now = datetime.utcnow()
    time_delta = timedelta(days=days, minutes=minutes)
    adjusted_time = now + time_delta
    epoch_microseconds = int(adjusted_time.timestamp()) * 1000000
    return epoch_microseconds


def epoch_in_seconds(days=0, minutes=0):
    now = datetime.utcnow()
    time_delta = timedelta(days=days, minutes=minutes)
    adjusted_time = now + time_delta
